...
Sub ApplyMemoryModel()
    ...
    If TapeKB<=0 Or TapeKB>UXM_MAX_TAPE_KB Then
        HadError=1
        ErrMsg="HATA: tape bellek 1.."+Str(UXM_MAX_TAPE_KB)+" KB araliginda olmali. Verilen="+Str(TapeKB)+" KB"
        Exit Sub
    End If
    If StackKB<=0 Or StackKB>UXM_MAX_STACK_KB Then
        HadError=1
        ErrMsg="HATA: stack bellek 1.."+Str(UXM_MAX_STACK_KB)+" KB araliginda olmali. Verilen="+Str(StackKB)+" KB"
        Exit Sub
    End If
    If DataKB<=0 Or DataKB>UXM_MAX_DATA_KB Then
        HadError=1
        ErrMsg="HATA: data bellek 1.."+Str(UXM_MAX_DATA_KB)+" KB araliginda olmali. Verilen="+Str(DataKB)+" KB"
        Exit Sub
    End If
    If QueueKB<=0 Or QueueKB>UXM_MAX_QUEUE_KB Then
        HadError=1
        ErrMsg="HATA: queue/fifo bellek 1.."+Str(UXM_MAX_QUEUE_KB)+" KB araliginda olmali. Verilen="+Str(QueueKB)+" KB"
        Exit Sub
    End If
    If TapeKB+StackKB+DataKB+QueueKB>UXM_MAX_TOTAL_KB Then
        HadError=1
        ErrMsg="HATA: toplam UXM bellek " + Str(UXM_MAX_TOTAL_KB) + " KB ust sinirini asiyor. Tape+Stack+Data+Queue="+Str(TapeKB+StackKB+DataKB+QueueKB)+" KB"
        Exit Sub
    End If
