...
64 KB memory hattına uygun en iyi kaynak ağacını seçici olarak kurar; v2 derleme/gate/test onarımlarını içerir.
...
class UXMA64KBuilder:
    ...
    def patch_file(self, path: Path, patch_fn, layer: str, note: str = "") -> None:
        ...
        if new == old:
            ...
            return
        if not self.dry_run:
            # UXM-A64 v2: backup dosyaları src içinde bırakılmaz; FreeBASIC include/lint karışmasın diye reports/patch_backups altına alınır.
            try:
                rel = path.relative_to(self.out)
            except ValueError:
                rel = Path(path.name)
            bak = self.out / "reports" / "patch_backups" / rel.with_suffix(rel.suffix + ".bak")
    ...
    def patch_native_compiler_header(self) -> None:
        ...
        def patch(txt: str) -> str:
            txt = re.sub(r'Const\s+UXM_VERSION\s+As\s+String\s*=.*',
                         'Const UXM_VERSION As String="3.3-UXM-A-64K-v2-stage12-v20-merged"', txt)
...
"""
    ...
    def patch_native_cli_64k(self) -> None:
        ...
        new_apply = r'''Sub ApplyMemoryModel()
...
End Sub'''
        ...
        self.patch_file(p, patch, "compiler/native", "native_cli.bas 64KB ust limit invariantı; daha küçük layout testleri kabul edilir.")
...
'''
        ...
        def patch_interp(txt: str) -> str:
            ...
            # UXM-A64 v2: runtime_io.bas zaten ux_putc/ux_getc/ux_runtime_error/ux_print_data_string saglar.
            # Interpreter icindeki tekrar eden callback govdeleri derleme hatasi uretmesin diye kaldirilir.
            for _name, _endkw in [
                ("ux_putc", "End Sub"),
                ("ux_getc", "End Function"),
                ("ux_runtime_error", "End Sub"),
                ("ux_print_data_string", "End Sub"),
            ]:
                txt = re.sub(r'(?is)\n\s*(?:Sub|Function)\s+' + _name + r'\b.*?' + _endkw + r'\s*', '\n', txt, count=1)
    ...
    def _memory_kb_from_uxm(self, path: Path) -> tuple[int, bool]:
        """Return (tape+stack+data KB, compatible<=64). Missing #memory is default-compatible."""
        txt = path.read_text(encoding=UTF8, errors="ignore")
        m = re.search(r'(?im)^\s*#memory\s+([^\n\r]+)', txt)
        if not m:
            return (64, True)
        vals = {"tape": 32, "stack": 8, "data": 24}
        for part in m.group(1).split(','):
            ...
            if k not in vals:
                continue
            vals[k] = self._parse_size_kb(v.strip())
        total = vals["tape"] + vals["stack"] + vals["data"]
        return (total, total <= 64)

    def _parse_size_kb(self, s: str) -> int:
        m = re.match(r'(?i)^\s*(\d+)\s*([kmg]?b?)?\s*$', s)
        if not m:
            return 0
        n = int(m.group(1)); unit = (m.group(2) or "kb").lower()
        if unit in ["", "k", "kb"]: return n
        if unit in ["m", "mb"]: return n * 1024
        if unit in ["g", "gb"]: return n * 1024 * 1024
        return n

    def _copy_test_pair(self, src_file: Path, dst_root: Path, src_root: Path) -> None:
        rel = src_file.relative_to(src_root)
        dst = dst_root / rel
        dst.parent.mkdir(parents=True, exist_ok=True)
        shutil.copy2(src_file, dst)
        exp = src_file.with_suffix('.expect')
        if exp.exists():
            dst_exp = dst.with_suffix('.expect')
            shutil.copy2(exp, dst_exp)
    ...
    def copy_tests_and_tools(self) -> None:
        if "final_tests" in self.packages:
            ...
            try:
                ...
                self.copy_tree(src, self.out / "tests/final_expected_all", "tests", "Final expected suite bütün kopya.")
                selected = self.out / "tests/a64_selected_final_expected"
                quarantine = self.out / "tests/quarantine_over64_final_expected"
                rows = []
                for uxm in sorted(src.rglob('*.uxm')):
                    total, ok = self._memory_kb_from_uxm(uxm)
                    self._copy_test_pair(uxm, selected if ok else quarantine, src)
                    rows.append({"source": str(uxm.relative_to(src)).replace('\\','/'), "memory_kb": total, "a64_ok": ok})
                csv_path = self.out / "reports" / "UXM_A64_TEST_CLASSIFICATION.csv"
                csv_path.parent.mkdir(parents=True, exist_ok=True)
                with csv_path.open('w', encoding='utf-8-sig', newline='') as f:
                    w = csv.DictWriter(f, fieldnames=["source", "memory_kb", "a64_ok"]); w.writeheader(); w.writerows(rows)
                self.actions.append(Action("tests", "CLASSIFY", str(src), str(selected), "OK", f"A64 compatible={sum(1 for r in rows if r['a64_ok'])}, quarantine={sum(1 for r in rows if not r['a64_ok'])}"))
            except Exception as e:
                self.copy_tree(pkg, self.out / "tests/final_expected_package", "tests", f"Final expected paket bütün kopya; sınıflama yapılamadı: {e}")
        else:
            self.actions.append(Action("tests", "SKIP", "<final_tests>", "tests/final_expected_all", "SKIP", "24 final expected paketi bulunamadı."))

        try:
            stage12_tests = self.find_dir_named(self.packages["stage12"], "uxm/tests")
            dst_all = self.out / "tests/stage12_all"
            self.copy_tree(stage12_tests, dst_all, "tests", "Stage12 test ağacı ayrı kopya.")
            dst_smoke = self.out / "tests/a64_smoke_from_stage12"
            for sub in ["native", "v33", "fp", "math", "matrix"]:
                subdir = stage12_tests / sub
                if subdir.exists():
                    for uxm in sorted(subdir.glob('*.uxm')):
                        _total, ok = self._memory_kb_from_uxm(uxm)
                        if ok:
                            self._copy_test_pair(uxm, dst_smoke / sub, subdir)
            self.actions.append(Action("tests", "COPY_CLASSIFIED", str(stage12_tests), str(dst_smoke), "OK", "Stage12 A64 uyumlu smoke testleri kopyalandı."))
        except Exception as e:
            self.actions.append(Action("tests", "SKIP", "stage12 uxm/tests", "tests/a64_smoke_from_stage12", "ERROR", str(e)))

        v19 = self.packages.get("v19")
        if v19:
            candidates = [p for p in v19.rglob("stage24_placeholder_v19") if p.is_dir()]
            if candidates:
                self.copy_tree(candidates[0], self.out / "tests/v19_runtime_services", "tests", "V19 gerçek runtime servis testleri.")
...
echo [1/4] Placeholder, include ve 64KB statik kapi
python tools\uxm_a64_static_gate.py
if errorlevel 1 goto fail

echo [2/4] FreeBASIC compile gate: native + runtime + interpreter
python tools\uxm_a64_compile_gate.py
if errorlevel 1 goto fail

echo [3/4] A64 uyumlu secili testlerde ASM uretim denemesi
python tools\uxm_a64_expected_runner.py --limit 50
if errorlevel 1 goto fail

echo [4/4] Bitti. Ayrintili loglar reports klasorune yazildi.
goto end

:fail
echo HATA: UXM-A-64K gate basarisiz. Loglar reports klasorunde.
exit /b 1
:end
endlocal
'''
...
if "Tape+Stack+Data toplam" not in txt and "64 KB ust" not in txt:
    errors.append("native_cli.bas 64KB ust-limit invariant mesajını taşımıyor")
comp = root/"src/compiler/native/uxm31_compiler_fb.bas"
txt = comp.read_text(errors="ignore") if comp.exists() else ""
if "Const UXM_MAX_TOTAL_KB As Long=64" not in txt:
    errors.append("uxm31_compiler_fb.bas UXM_MAX_TOTAL_KB=64 taşımıyor")
...
'''
        ...
        compile_py = r'''#!/usr/bin/env python3
from pathlib import Path
import subprocess, shutil, sys, json
root = Path(__file__).resolve().parents[1]
reports = root / "reports" / "compile_logs"
reports.mkdir(parents=True, exist_ok=True)
fbc = shutil.which("fbc")
if not fbc:
    print("HATA: fbc PATH içinde bulunamadı.")
    sys.exit(2)
commands = [
    [fbc, str(root/"src/uxm_v20_native_compiler.bas"), "-x", str(root/"build/uxm_a64_native_compiler.exe")],
    [fbc, str(root/"src/uxm_v20_runtime.bas"), "-c", "-o", str(root/"build/uxm_a64_runtime.o")],
    [fbc, str(root/"src/interpreter/uxm_v20_interpreter.bas"), "-x", str(root/"build/uxm_a64_interpreter.exe")],
]
summary=[]
for idx, cmd in enumerate(commands,1):
    name = f"step_{idx}_{Path(cmd[1]).stem}.log"
    print("RUN:", " ".join(cmd))
    p = subprocess.run(cmd, cwd=root, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    (reports/name).write_text(p.stdout, encoding="utf-8", errors="ignore")
    summary.append({"cmd":cmd,"returncode":p.returncode,"log":str((reports/name).relative_to(root))})
    if p.returncode != 0:
        (reports/"compile_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
        print(p.stdout)
        sys.exit(p.returncode)
(reports/"compile_summary.json").write_text(json.dumps(summary, indent=2), encoding="utf-8")
print("OK: FreeBASIC compile gate geçti.")
'''
        self.write_text(self.out / "tools/uxm_a64_compile_gate.py", compile_py, "tools", "FreeBASIC compile logs gate.")

        expected_runner = r'''#!/usr/bin/env python3
from pathlib import Path
import argparse, subprocess, sys, json
root = Path(__file__).resolve().parents[1]
parser = argparse.ArgumentParser()
parser.add_argument("--tests", default="tests/a64_selected_final_expected")
parser.add_argument("--limit", type=int, default=50)
parser.add_argument("--compiler", default=str(root/"build/uxm_a64_native_compiler.exe"))
args = parser.parse_args()
compiler = Path(args.compiler)
if not compiler.exists():
    print("HATA: native compiler exe bulunamadı:", compiler)
    sys.exit(2)
test_root = root / args.tests
out_root = root / "reports" / "asm_generation_logs"
out_root.mkdir(parents=True, exist_ok=True)
rows=[]
for n, uxm in enumerate(sorted(test_root.rglob("*.uxm")), 1):
    if args.limit and n > args.limit: break
    asm = out_root / (uxm.stem + ".asm")
    log = out_root / (uxm.stem + ".log")
    p = subprocess.run([str(compiler), str(uxm), str(asm)], cwd=root, text=True, stdout=subprocess.PIPE, stderr=subprocess.STDOUT)
    log.write_text(p.stdout, encoding="utf-8", errors="ignore")
    rows.append({"test":str(uxm.relative_to(root)), "returncode":p.returncode, "asm":str(asm.relative_to(root)), "log":str(log.relative_to(root))})
    if p.returncode != 0:
        print("FAIL", uxm, "->", log)
        break
(out_root/"asm_generation_summary.json").write_text(json.dumps(rows, indent=2), encoding="utf-8")
print("ASM generation tested:", len(rows), "failures:", sum(1 for r in rows if r['returncode']!=0))
sys.exit(1 if any(r['returncode']!=0 for r in rows) else 0)
'''
        self.write_text(self.out / "tools/uxm_a64_expected_runner.py", expected_runner, "tools", "A64 selected tests ASM generation runner.")
    ...
    def collect_source_metrics(self) -> list[dict]:
        ...
        for p in sorted((self.out / "src").rglob("*")):
            if not p.is_file():
                continue
            if p.name.endswith(".bak") or p.suffix.lower() not in [".bas", ".bi", ".ts", ".json"]:
                continue
    ...
    def check_declared_implemented(self) -> tuple[list[str], list[str]]:
        ...
        for p in (self.out / "src").rglob("*"):
            if p.name.endswith(".bak") or p.suffix.lower() not in [".bas", ".bi"] or not p.is_file():
                continue
            txt = p.read_text(encoding=UTF8, errors="ignore")
