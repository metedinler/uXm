...
& powershell.exe -NoProfile -ExecutionPolicy Bypass -File $Merkez -Komut 'hizli_tara' -Kok $Root -DerlemeYok:$NoBuild -IlkHata:$StopOnFail -Adet $Limit -Basla $FromIndex -Ara $NameContains -Zaman $TimeoutTest -Uygula:$Apply -BuildEmekli:$RetireBuild
exit $LASTEXITCODE
