# UXM V3.3 Stage-6 — UX-STR V2 Genişletilmiş Text Servisleri
...
Stage-6 ayrıca `FirstPassDefinitions` davranışını düzeltti. Eski ilk geçiş, normal kod içindeki `(SP-1)` ifadesinde `S` harfini `sN` string tanımı gibi görüyordu. Artık ilk geçiş yalnızca gerçek `sN` / `mN` satırlarını toplar ve diğer kod satırlarını atlar.
