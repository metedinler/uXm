Tamam Mete abi. **Önceki QB64 + FreeBASIC runtime sürümü ilk çalışan iskelet seviyesinde bitti**, ama V3.1’in bütün kurallarını tam taşıyan son sürüm değildi. Şimdi bunu **tamamen FreeBASIC tarafına çekiyorum**. İlk dosya: **FreeBASIC ile yazılmış compiler**. Bu dosya `.uxm` okur, V3.1 kurallarına göre parse eder ve Windows x64 NASM `.asm` üretir.

Kod uzun olduğu için önce compiler dosyasını veriyorum. Sonraki mesajda runtime’ı vereceğim.

# Dosya 1: `uxm31_compiler_fb.bas`
...
Bu dosya **compiler tarafıdır**. Şimdi sıra ikinci dosyada: **tam FreeBASIC runtime**.
