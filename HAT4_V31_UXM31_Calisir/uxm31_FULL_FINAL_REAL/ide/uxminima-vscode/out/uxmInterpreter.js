...
class UxmInterpreter {
    ...
    resolve(addr) { let idx = 0; switch (addr.kind) {
        case "T": return ["T", this.ptr];
        case "T_REL": return ["T", this.ptr + addr.value];
        case "T_ABS": return ["T", addr.value];
        case "D_ABS": return ["D", addr.value];
        case "S_ABS": return ["S", addr.value];
        case "SP": return ["S", Math.max(0, this.sp - 1)];
        case "P": return ["P", 0];
        case "E": return ["E", 0];
        case "F": return ["F", 0];
        case "IND_T":
